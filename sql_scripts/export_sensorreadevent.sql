SELECT 
	DATE_FORMAT(CONVERT_TZ(timestamp,'+00:00','-03:00'), "%Y-%m-%d %H:%i:%s") AS 'data/hora',
...
WHERE 
   ...
   ((sensor_id IN (7, 15) AND value_read <> 0) OR (sensor_id NOT IN (7, 15))) AND
   ((sensor_id IN (6, 14) AND value_read <> 0) OR (sensor_id NOT IN (6, 14))) AND
   ((sensor_id IN (17, 18) AND value_read <> 9) OR (sensor_id NOT IN (17, 18)))
ORDER BY timestamp;